#import "../../../lib.typ":*
== Generalization of Important Properties 
...
+ Continuous Functions
  #definition[
    Let $f:X |-> Y$, each with metric $(X, d_x)$ and $(Y, d_y)$. $f$ is continuous iff $
      forall epsilon in RR^+, quad forall x_1, x_2 in X, quad exists delta in RR^+: d_x (x_1, x_2) < delta ==> d_y (f(x_1), f(x_2))< epsilon.
    $
  ]
  #remark[
    In here, we similarly generalized the absolute value in the classic real analysis definition to $d$.
...
#proof[
  Here, we directly invoke the definition of continuity of operators. It suffices to show that $
      forall epsilon in RR^+, quad forall f, g in C^1 ([a, b]), quad exists delta in RR^+: d_C_1 (f, g) < delta ==> d_C_0 (f', g')< epsilon.
    $
...
#solution[
  ...
  Assume for contradiction that $f!= g$, then $ exists x_0 in [a, b]: f(x_0) != g(x_0). $ WLOG, $f(x_0) > g(x_0) $, then $
  exists r in RR: forall x_1 in B_r (x_0), quad f(x_1) > g(x_1).   $ Then, $ integral_a^b abs(f(x) - g(x)) dif x > 0, $ contradiction. 
