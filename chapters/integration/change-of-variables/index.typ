...
#lbl(<ex:integration-change-of-variables-1>, example[
...
#lbl(<rem:integration-change-of-variables-1>, remark[
...
Since linear transformations preserve collinearity, a $dif x$ by $dif y$ rectangle is mapped to a parallelogram, with area $ dif u times dif v & = ( u_x dif x + u_y dif y ) times (v_x dif x + v_y dif y) \
                  & = (u_x v_y - v_x u_y) dif x times dif y \
                  & = mat(u_x, u_y; v_x, v_y; delim: "|") dif x times dif y. $

#lbl(<def:integration-change-of-variables-1>, definition[
...
#lbl(<def:integration-change-of-variables-2>, definition[
...
#lbl(<ex:integration-change-of-variables-2>, example[
  Find the jacobian of $x$ and $y$ wrt $r$ and $theta$.
])
#solution[
  By definition, $ bold(J) & = (partial (x, y))/(partial (r, theta)) \
          & = mat(x_r, x_theta; y_r, y_theta; delim: "|") \
          & =mat(cos theta, - r sin theta; sin theta, r cos theta; delim: "|") \
          & = r cos^2 theta + r sin^2 theta \
          & = r. $

  Furthermore, $ dif x dif y = r dif r dif theta, $ supporting the previous geometric argument.
]
#lbl(<rem:integration-change-of-variables-2>, remark[
...
#lbl(<ex:integration-change-of-variables-3>, example[
  Compute $ integral_0^1 integral_0^1 x^2 y dif x dif y, $ with the change of variables of $ cases(u= x, v = x y). $
])

#solution[
  We first compute the jacobian: $ bold(J) & = mat(u_x, u_y; v_x, v_y; delim: "|") \
          & = mat(1, 0; y, x; delim: "|") \
          & = x. $
